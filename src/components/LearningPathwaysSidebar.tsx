...
interface LearningPathwaysSidebarProps {
  isMobile?: boolean;
  isOpen?: boolean;
        ...
        {pathways.map((pathway) => (
          ...
          >
            <div className="flex items-start gap-3">
              {/* Icon */}
              <div className={cn(
                "p-2.5 rounded-lg bg-gradient-to-br flex-shrink-0",
                pathway.color,
...
export function LearningPathwaysSidebar({ isMobile, isOpen, onOpenChange }: LearningPathwaysSidebarProps) {
  if (isMobile) {
    return (
...
export function LearningPathwaysTrigger({ onClick }: { onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={cn(
